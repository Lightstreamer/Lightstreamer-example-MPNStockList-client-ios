...
class StockListViewController: UITableViewController, SubscriptionDelegate, UIPopoverControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func connectionStatusChanged() {
        ...
        let needsMPNUnsubscription = Connector.shared().connected
        ...
        if needsMPNUnsubscription && Connector.shared().mpnEnabled {
            print("StockListViewController: unsubscribing triggered MPNs...")

            Connector.shared().unsubscribeTriggeredMPNs()
        }
    }
    ...
    // MARK: -
    // MARK: Communication with App delegate
    @objc func handleMPN(_ mpn: [AnyHashable : Any]?) {
        // This method is always called from the main thread

        // Extract MPN content
        let item = mpn?["item"] as? String
        let aps = mpn?["aps"] as? [AnyHashable : Any]
        let alert = aps?["alert"] as? [AnyHashable : Any]
        let body = alert?["body"] as? String

        // Skip extraneous MPN notifications
        let index = TABLE_ITEMS.firstIndex(of: item ?? "") ?? NSNotFound
        if index == NSNotFound {
            return
        }

        // Check if we are watching this item or another one
        let itemIsSelected = selectedRow != nil && (selectedRow?.row == index)
        if itemIsSelected {
        ...
        if itemIsSelected {
            ...
            alertBox.addAction(UIAlertAction(
                                ...
                                handler: { _ in
                                    ...
                                    if let table = self.stockListView?.table {
                                        self.tableView(table, willSelectRowAt: IndexPath(row: index, section: 0))
                                    }
                                }))
            alertBox.addAction(UIAlertAction(
                                title: "Skip",
    ...
    @objc func appDidBecomeActive() {
        ...
        if selectedRow != nil && (DEVICE_IPAD || ((navigationController?.viewControllers.count ?? 0) > 1)) {
