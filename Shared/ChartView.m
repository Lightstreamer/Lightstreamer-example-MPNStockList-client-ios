...
#define THRESHOLD_LINE_COLOR        ([UIColor redColor])
#define THRESHOLD_LABEL_COLOR       ([UIColor redColor])

#define AXIS_LINE_WIDTH              (2.0)
#define INTEGER_LINE_WIDTH           (1.0)
#define VALUE_LINE_WIDTH             (2.0)
#define THRESHOLD_LINE_WIDTH         (1.0)
...
- (id) initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		_thresholds= [[NSMutableArray alloc] init];
...
- (void) drawRect:(CGRect)rect {
	...
	@synchronized (self) {
		...
		// Draw thresholds
		CGContextSetStrokeColorWithColor(context, THRESHOLD_LINE_COLOR.CGColor);
		CGContextSetLineWidth(context, THRESHOLD_LINE_WIDTH);
		...
		for (ChartThreshold *threshold in _thresholds) {
			...
			// Draw label
			NSString *label= [NSString stringWithFormat:@"%7.2f", value];
			NSAttributedString *labelAttr= [[NSAttributedString alloc] initWithString:label attributes:attributesDict];
			CGContextSetTextPosition(context, 0.0, y);
			line= CTLineCreateWithAttributedString((CFAttributedStringRef) labelAttr);
			CTLineDraw(line, context);
