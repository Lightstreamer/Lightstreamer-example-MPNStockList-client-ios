...
class ChartView: UIView {
    ...
    @objc func addThreshold(_ value: Float) -> ChartThreshold? {
        let threshold = ChartThreshold(view: self, value: value)
    ...
    @objc func findThreshold(within margin: Float, fromValue value: Float) -> ChartThreshold? {
        return lockQueue.sync {
            for threshold in thresholds ?? [] {
                guard let threshold = threshold as? ChartThreshold else {
                    continue
                }
                if Float(abs(threshold.value - value)) < margin {
                    return threshold
                }
            }
            return nil
    ...
    @objc func remove(_ threshold: ChartThreshold?) {
        lockQueue.sync {
            thresholds?.removeAll { $0 as AnyObject === threshold as AnyObject }
        }

        setNeedsDisplay()
    }

    @objc func clearThresholds() {
        lockQueue.sync {
    ...
    override func draw(_ rect: CGRect) {
        ...
        lockQueue.sync {
            ...
            // Draw thresholds
            context?.setStrokeColor(THRESHOLD_LINE_COLOR.cgColor)
            context?.setLineWidth(CGFloat(THRESHOLD_LINE_WIDTH))

            if let sysBoldFont = sysBoldFont {
                attributesDict = [
                    kCTFontAttributeName : sysBoldFont,
                    kCTForegroundColorAttributeName : THRESHOLD_LABEL_COLOR.cgColor
                ]
            }
            ...
            for threshold in thresholds ?? [] {
                ...
                let point = self.point(forValue: CGPoint(x: 0.0, y: CGFloat(value)))
                let y = Float(point.y)
                ...
                // Draw label
                let label = String(format: "%7.2f", value)
                let labelAttr = NSAttributedString(string: label, attributes: attributesDict as? [NSAttributedString.Key : Any])
                context?.textPosition = CGPoint(x: 0.0, y: CGFloat(y))
                line = CTLineCreateWithAttributedString(labelAttr as CFAttributedString)
                if let context = context {
